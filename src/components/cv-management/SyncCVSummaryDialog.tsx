...
export default function SyncCVSummaryDialog({
  ...
  const [showSuggestions, setShowSuggestions] = useState<{ type: 'core' | 'soft'; index: number } | null>(null);
  ...
  const handleSkillInputChange = async (value: string, type: 'core' | 'soft', index: number) => {
    // Update the skill value first
    if (type === 'core') {
    ...
    try {
      const suggestions = await fetchSkillSuggestions(value, type);
  ...
  const handleSelectSuggestion = (suggestion: string, type: 'core' | 'soft', index: number) => {
    if (type === 'core') {
      const skills = [...(editedData?.core_skills || [])];
      skills[index] = { ...skills[index], skill: suggestion };
      updateField('core_skills', skills);
    } else {
      const skills = [...(editedData?.soft_skills || [])];
      skills[index] = suggestion;
      updateField('soft_skills', skills);
    }
    setShowSuggestions(null);
    setSkillSuggestions([]);
                        ...
                        <div key={index} className="flex items-center gap-2 relative">
                          <div className="relative flex-1">
                            <Input
                              ...
                              onChange={(e) => handleSkillInputChange(e.target.value, 'core', index)}
                              onFocus={() => {
                                if (skillSuggestions.length > 0 && showSuggestions?.type === 'core' && showSuggestions?.index === index) {
                                  setShowSuggestions({ type: 'core', index });
                                }
                              }}
                              placeholder="Type to search skills..."
                              className="flex-1"
                            />
                            {showSuggestions?.type === 'core' && showSuggestions?.index === index && skillSuggestions.length > 0 && (
                              <div
                                ref={suggestionsRef}
                                ...
                                {isLoadingSuggestions ? (
                                  ...
                                  skillSuggestions.map((suggestion, idx) => (
                                    <button
                                      ...
                                      onClick={() => handleSelectSuggestion(suggestion.name, 'core', index)}
                                    >
                                      {suggestion.name}
                                    </button>
                                  ))
                      ...
                      {(editedData.soft_skills || []).map((skill: string, index: number) => (
                        <div key={index} className="flex items-center gap-2 relative">
                          <div className="relative flex-1">
                            <Input
                              value={skill}
                              onChange={(e) => handleSkillInputChange(e.target.value, 'soft', index)}
                              onFocus={() => {
                                if (skillSuggestions.length > 0 && showSuggestions?.type === 'soft' && showSuggestions?.index === index) {
                                  setShowSuggestions({ type: 'soft', index });
                                }
                              }}
                              placeholder="Type to search skills..."
                              className="flex-1"
                            />
                            {showSuggestions?.type === 'soft' && showSuggestions?.index === index && skillSuggestions.length > 0 && (
                              <div
                                ref={suggestionsRef}
                                className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto"
                              >
                                {isLoadingSuggestions ? (
                                  ...
                                  skillSuggestions.map((suggestion, idx) => (
                                    <button
                                      ...
                                      type="button"
                                      className="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 focus:bg-gray-100 focus:outline-none"
                                      onClick={() => handleSelectSuggestion(suggestion.name, 'soft', index)}
                                    >
                                      {suggestion.name}
                                    </button>
                                  ))
