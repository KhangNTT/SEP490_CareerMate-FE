...
export default function RecruiterConfirmPaymentPage() {
  ...
  const packageName = searchParams.get('package') as RecruiterPackageName;
  
  const [isLoading, setIsLoading] = useState(false);
  const [packages, setPackages] = useState<RecruiterPackage[]>([]);
  const [selectedPackage, setSelectedPackage] = useState<RecruiterPackage | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const pkg = getRecruiterPackage(packagesData, packageName);
  ...
  const handleConfirmPurchase = async () => {
    ...
    // If BASIC package, no payment needed
    if (selectedPackage.name === 'BASIC') {
      toast.success('BASIC package activated!');
      router.push('/payment/success?package=BASIC');
      return;
    }
    ...
    try {
      // Create payment URL
      const paymentUrl = await createRecruiterPaymentUrl(selectedPackage.name);
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-8">
              ...
              <p className="text-gray-500 mb-8 text-sm">
                You are about to upgrade your recruiter account. Please review the details below.
              </p>
              ...
              <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 mb-8">
                ...
                <div>
                  ...
                  <ul className="grid gap-3">
                    {selectedPackage && formatRecruiterPackageFeatures(selectedPackage.entitlements).map((feature, index) => (
          ...
          <div className="md:col-span-1">
            <div className="bg-white rounded-3xl shadow-lg border border-gray-200 p-6 sticky top-8">
              ...
              <Button
                ...
                className="w-full py-6 text-base rounded-xl shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white"
                disabled={isLoading}
              >
                {isLoading ? (
